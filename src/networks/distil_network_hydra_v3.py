...
class LLL_Net_Hydra_v3(nn.Module):
    """DeiT backbone + 6 head lists for vit_hydra_v3 CIL.

    cls_features  -> heads              (per task)  — original
    dist_features -> heads_dist         (per task)  — original
    cls_features  -> mlp_cls  -> heads_mlp_cls   (per task)  — ConvNeXt, standard  4×768
    dist_features -> mlp_dist -> heads_mlp_dist  (per task)  — Swin,     narrow    2×768
    cls_features  -> mlp_res  -> heads_mlp_res   (per task)  — ResNet101, wide     8×768
    dist_features -> mlp_eff  -> heads_mlp_eff   (per task)  — EfficientNet, deeper 4×768 ×3

    All 4 MLPs loaded from mlp_weights_v3.pth and kept frozen.
    """
    ...
    def __init__(self, model, mlp_weights_path=None):
        ...
        self.mlp_cls  = MLP(self.out_size, hidden_dim=4 * self.out_size, num_layers=2)
        self.mlp_dist = MLP(self.out_size, hidden_dim=2 * self.out_size, num_layers=2)
        self.mlp_res  = MLP(self.out_size, hidden_dim=8 * self.out_size, num_layers=2)
        self.mlp_eff  = MLP(self.out_size, hidden_dim=4 * self.out_size, num_layers=3)
