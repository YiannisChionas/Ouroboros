...
class Appr(Incremental_Learning_Approach):
    ...
    - dist token, old tasks: cosine similarity loss on backbone features (student vs teacher dist_feat)
    ...
    Training the dist head with CE (as in cos_v1) and then ignoring it at inference
    adds noise without benefit. Here the dist head plays no role in either training
    or inference for the current task.
    """
    ...
    def eval(self, t, val_loader):
        with torch.no_grad():
            ...
            stats = {i: {'cls_max': 0, 'cls_norm': 0, 'count': 0} for i in range(t + 1)}
            ...
            print("\n" + "=" * 40)
            print(f"DEBUG: CLS Logit Statistics at Task {t}")
            for task_id in range(t + 1):
                s = stats[task_id]
                c = s['count']
