...
class Appr(Incremental_Learning_Approach):
    """Hydra v3: Weight Alignment on frozen dist_heads for calibrated retrieval.

    After training dist_head[t], its weight norms are aligned to the mean norm
    of previously frozen dist_heads. This ensures all task detectors operate at
    the same confidence scale, preventing newer heads from dominating retrieval.

    cls_head  → in-task classifier (standard CE + KD)
    dist_head → task identifier (trained with CE, WA-aligned, then frozen)
    ...
    After each task:
      1. Weight-align dist_head[t] to previous heads (if t > 0)
      2. Freeze dist_head[t]
    ...
    def _align_dist_head(self, t):
        """Weight Alignment: scale dist_head[t] weights to match mean norm of heads[0..t-1]."""
        if t == 0:
            return
        with torch.no_grad():
            prev_norms = []
            for k in range(t):
                w = self.model.heads_dist[k].weight  # [n_cls, D]
                prev_norms.append(w.norm(dim=1).mean())
            target_norm = torch.stack(prev_norms).mean()

            w = self.model.heads_dist[t].weight
            current_norm = w.norm(dim=1).mean()
            scale = target_norm / (current_norm + 1e-8)

            self.model.heads_dist[t].weight.mul_(scale)
            if self.model.heads_dist[t].bias is not None:
                self.model.heads_dist[t].bias.mul_(scale)

    def post_train_process(self, t, trn_loader):
        # Align dist_head[t] norms to previous frozen heads, then freeze
        self._align_dist_head(t)
