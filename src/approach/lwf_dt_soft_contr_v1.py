...
class Appr(Incremental_Learning_Approach):
    """
    LwF with InfoNCE contrastive distillation on the dist token.

    - cls head, old tasks : soft KL distillation on logits (teacher cls_logits)
    - dist token, old tasks: InfoNCE contrastive loss on backbone features
                             (student dist_feat as query, teacher dist_feat of same
                             sample as positive, other samples in batch as negatives)
    - current task (cls + dist): CE with ground truth
    - inference: cls logits only

    Motivation: contrastive loss not only pulls student features toward their teacher
    counterpart but also pushes them away from other samples' teacher features,
    preserving the relative geometry of the representation space more explicitly
    than a simple cosine similarity loss.
    """
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--lamb-contr', default=1, type=float, required=False,
                            help='Weight for InfoNCE contrastive loss on dist token features (default=%(default)s)')
        parser.add_argument('--tau', default=0.07, type=float, required=False,
                            help='Temperature for InfoNCE contrastive loss (default=%(default)s)')
    ...
    def eval(self, t, val_loader):
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                # Diagnostic: cosine similarity between student and teacher dist features
                if t > 0:
            ...
            if t > 0:
                ...
                print(f"  AvgCosSim : {feat_cos_sim_sum / feat_count:.4f}  (diagnostic only — loss is InfoNCE)")
    ...
    def infonce_loss(self, student_feat, teacher_feat):
        """
        InfoNCE contrastive loss.

        For each sample i in the batch:
          - positive: teacher_feat[i] (same sample, different model)
          - negatives: teacher_feat[j] for j != i (other samples)

        Loss = -log( exp(sim(s_i, t_i) / tau) / sum_j exp(sim(s_i, t_j) / tau) )
             = CrossEntropy(similarity_matrix / tau, diagonal_labels)
        """
        s = F.normalize(student_feat, dim=-1)   # [B, D]
        t = F.normalize(teacher_feat, dim=-1)   # [B, D]
        logits = torch.matmul(s, t.T) / self.tau  # [B, B]
    ...
    def criterion(self, t, outputs, targets, outputs_old):
        ...
        if t > 0:
            ...
            # --- Loss 2: InfoNCE contrastive on dist token features ---
            # Pulls student features toward their teacher counterpart
            # while pushing away from other samples' teacher features.
            loss += self.lamb_contr * self.infonce_loss(
                outputs["dist_features"],
                outputs_old["dist_features"]
            )
