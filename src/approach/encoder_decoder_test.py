...
class Appr(Incremental_Learning_Approach):
    """Encoder-decoder task retrieval — built on lwf_dt_cos_v2 training.

    Training (identical to lwf_dt_cos_v2):
      - cls head, old tasks : soft KL distillation on cls logits
      - dist token, old tasks: cosine similarity loss on backbone features
      - current task        : CE with GT on cls head only

    After each task: store mean dist_features over training set as prototype.
