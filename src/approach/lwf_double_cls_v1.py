...
class Appr(Incremental_Learning_Approach):
    """
    Ablation of lwf_dt_soft_cos_v2: two heads, both fed cls features only (no dist token).

    Training (both heads, same loss):
    - old tasks : soft KL distillation on logits (teacher cls_logits as target for both)
    - current task: CE with ground truth on both heads

    Inference: cls logits only.

    Purpose: if this is worse than cos_v2, the dist token in DeiT provides real benefit
    beyond having two heads.
    """
    ...
    def extra_parser(args):
        ...
        parser.add_argument('--lamb', default=1, type=float, required=False,
                            help='Weight for KL distillation loss on logits (default=%(default)s)')
    ...
    def criterion(self, t, outputs, targets, outputs_old):
        ...
        if t > 0:
            old_cls_target = torch.cat(outputs_old["cls_logits"][:t], dim=1)

            # --- Loss 1: soft KL on cls head for old tasks ---
            ...
            # --- Loss 2: same soft KL on dist head for old tasks (same target) ---
            loss += self.lamb * self.cross_entropy(
        ...
        # --- Loss 3: CE with GT on current task — cls head ---
        loss += F.cross_entropy(cls_logits[t], targets - self.model.task_offset[t])

        # --- Loss 4: CE with GT on current task — dist head (same loss) ---
