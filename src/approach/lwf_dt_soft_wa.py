...
class Appr(Incremental_Learning_Approach):
    ...
    def post_train_process(self, t, trn_loader):
        """Runs after training all the epochs of the task"""
        
        # 1. Εφαρμογή Weight Aligning (WA) για τη διόρθωση του Recency Bias
        if t > 0:
            print(f"Applying Weight Aligning for task {t}...")
            self.apply_weight_aligning(t)

        # 2. Restore best and save model for future tasks
        self.model_old = deepcopy(self.model)
        self.model_old.eval()
        self.model_old.freeze_all()

    def apply_weight_aligning(self, t):
        with torch.no_grad():
            weights = [self.model.heads[i].weight for i in range(t + 1)]
            
            norms_old = torch.cat([torch.norm(w, p=2, dim=1) for w in weights[:t]])
            mean_norm_old = norms_old.mean()
            
            norm_new = torch.norm(weights[t], p=2, dim=1)
            mean_norm_new = norm_new.mean()
            
            gamma = mean_norm_old / mean_norm_new
            
            print(f"  WA: Mean Norm Old = {mean_norm_old:.4f}")
            print(f"  WA: Mean Norm New = {mean_norm_new:.4f}")
            print(f"  WA: Scaling Task {t} weights by gamma = {gamma:.4f}")
            
            self.model.heads[t].weight *= gamma
