...
class Appr(Incremental_Learning_Approach):
    """Hydra v5: v4 without KD on cls logits and without CE on dist head.

    Compared to v4:
      - cls_head, old tasks  : NO KD (removed)
      ...
      - cls_head, current    : CE with GT (only classification supervision)
      - dist_head, current   : NO CE (removed)

    Hypothesis: dist-based backbone regularization alone is sufficient to keep
    cls heads stable, without explicitly constraining cls logit outputs.

    After each task: freeze dist_head[t].

    Inference: same as v4 — retrieval via frozen dist_heads, classify via cls_heads.
    """
    ...
    def criterion(self, t, outputs, targets, outputs_old):
        ...
        # CE only on cls head for current task
