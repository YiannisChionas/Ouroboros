#!/usr/bin/env python3
"""Mini-pretraining of VitHydra_v3 MLP heads using four teacher networks.

Freezes DeiT backbone and trains:
  mlp_cls  + fc_cls  <- soft KD from Teacher A (convnext_base,    cls_features)
  mlp_dist + fc_dist <- soft KD from Teacher B (swin_base,        dist_features)
  mlp_res  + fc_res  <- soft KD from Teacher C (resnet101,        cls_features)
  mlp_eff  + fc_eff  <- soft KD from Teacher D (efficientnet_b4,  dist_features)

Loss: DeiT soft distillation (KL with log_target=True, T^2 scaling). No CE term.

Usage:
  python pretrain_vit_hydra_v3.py --data /opt/storage/datasets/imagenet1k --output mlp_weights_v3.pth
"""
...
import networks.deit_original
from networks.vit_hydra_v3 import VitHydra_v3


TEACHER_A = 'convnext_base.fb_in1k'
TEACHER_B = 'swin_base_patch4_window7_224.ms_in1k'
TEACHER_C = 'resnet101.a1_in1k'
TEACHER_D = 'efficientnet_b4.ra2_in1k'
...
def main():
    ...
    parser.add_argument('--output',      default='mlp_weights_v3.pth')
    ...
    teacher_c = timm.create_model(TEACHER_C, pretrained=True).to(device).eval()
    teacher_d = timm.create_model(TEACHER_D, pretrained=True).to(device).eval()
    for t in [teacher_a, teacher_b, teacher_c, teacher_d]:
        for p in t.parameters(): p.requires_grad = False

    print('Loading backbone...')
    backbone = timm.create_model(BACKBONE, pretrained=True)
    model = VitHydra_v3(backbone, teacher_out_dim=1000).to(device)
    model.freeze_backbone()

    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f'Trainable params: {trainable / 1e6:.1f}M (4 MLPs + 4 FC heads)')
    ...
    if args.resume and os.path.exists(ckpt_path):
        ...
        model.mlp_res.load_state_dict(ckpt['mlp_res'])
        model.mlp_eff.load_state_dict(ckpt['mlp_eff'])
        model.fc_cls.load_state_dict(ckpt['fc_cls'])
        model.fc_dist.load_state_dict(ckpt['fc_dist'])
        model.fc_res.load_state_dict(ckpt['fc_res'])
        model.fc_eff.load_state_dict(ckpt['fc_eff'])
    ...
    for epoch in range(start_epoch, end_epoch):
        ...
        for i, (images, _) in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                logits_c = teacher_c(images)
                logits_d = teacher_d(images)

            out = model(images)
            loss = (
                kd_loss(out['fc_cls_logits'],  logits_a, T) +
                kd_loss(out['fc_dist_logits'], logits_b, T) +
                kd_loss(out['fc_res_logits'],  logits_c, T) +
                kd_loss(out['fc_eff_logits'],  logits_d, T)
            )
        ...
        torch.save({
            ...
            'mlp_res':    model.mlp_res.state_dict(),
            'mlp_eff':    model.mlp_eff.state_dict(),
            'fc_cls':     model.fc_cls.state_dict(),
            'fc_dist':    model.fc_dist.state_dict(),
            'fc_res':     model.fc_res.state_dict(),
            'fc_eff':     model.fc_eff.state_dict(),
